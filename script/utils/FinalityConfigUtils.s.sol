...
library FinalityConfigUtils {
    ...
    function _encode(bool waitForSafe, uint256 blockDepth) internal pure returns (bytes4) {
        require(
    ...
    function _parseDeclared(string memory json, string memory basePath) internal view returns (bytes4) {
        bool waitForSafe;
        uint256 blockDepth;
    ...
    function _decodeModeLabel(bytes4 config) internal pure returns (string memory) {
    ...
    function _logFinalityConfig(bytes4 config) internal pure {
