...
/// @notice Deploys a SiloedLockRelease token pool and records it in the registry. It takes no lock box: map
/// remote chains to ERC20LockBoxes afterwards with configure/siloed/ConfigureLockBoxes.s.sol, before wiring
/// lanes. DECIMALS=<n> is required only when the token does not answer decimals(), and must agree with it
/// when both exist; the resolved value is the pool's immutable scaling factor.
contract DeploySiloedLockReleaseTokenPool is Script {
    ...
    function run() external {
        ...
        console.log("");
        console.log("========================================");
        console.log(unicode"🔐 Deploy Siloed Lock & Release Token Pool");
        console.log("========================================");
        console.log(string.concat("Chain:        ", chainName));
        console.log(string.concat("Action:       ", "Deploy siloed lock & release token pool"));
        console.log("========================================");
        console.log("");
        ...
        RegistryWriter._guard(selectorName, DeploymentRecorder._poolName(symbol, "SiloedLockRelease"));

        vm.startBroadcast();

        console.log(string.concat("\n[Step 1] Deploying SiloedLockReleaseTokenPool on ", chainName));
        SiloedLockReleaseTokenPool tokenPool =
            new SiloedLockReleaseTokenPool(IERC20(tokenAddress), decimals, poolHooks, config.rmnProxy, config.router);
        ...
        OutcomeLog._sending("deploy the SiloedLockRelease token pool");
        ...
        string memory expectedTypeAndVersion =
            string.concat("SiloedLockReleaseTokenPool ", DeploymentRecorder.POOL_VERSION);
        require(
        ...
        console.log("");
        console.log("========================================");
        OutcomeLog._sending(string.concat("deploy the SiloedLockRelease token pool on ", chainName));
        console.log("========================================");
        console.log(string.concat("Token Pool Address: ", vm.toString(tokenPoolAddress)));
        console.log(helperConfig.getExplorerUrl(chainId, "/address/", tokenPoolAddress));
        console.log("");
        // Single writer: ledger file + deployments[{symbol}_SiloedLockReleaseTokenPool_{version}] + active.tokenPool.
        DeploymentRecorder._recordTokenPool(
            vm, selectorName, config.chainNameIdentifier, tokenPoolAddress, tokenAddress, "SiloedLockRelease"
        );
        console.log("");
