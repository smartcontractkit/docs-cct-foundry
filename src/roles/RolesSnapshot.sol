...
contract RolesSnapshot {
    ...
    function _firstAdminCandidate(Ctx memory c, string memory json) private view returns (address) {
        ...
        for (uint256 i = 0; i < candidates.length; i++) {
            if (RolesProbes._hasRole(c.token, RolesProbes.DEFAULT_ADMIN_ROLE, candidates[i])) {
                return candidates[i];
            }
        }
    ...
    function _mintBurnBlock(Ctx memory c, string memory json, bool minters) private returns (string memory) {
        if (minters) {
            return _holdersBlock(
                ...
                "minters",
                "getMinters()",
                RolesProbes._roleIdOrDefault(c.token, "MINTER_ROLE()", RolesProbes.MINTER_ROLE),
        ...
        return _holdersBlock(
            ...
            "burners",
            "getBurners()",
            RolesProbes._roleIdOrDefault(c.token, "BURNER_ROLE()", RolesProbes.BURNER_ROLE),
    ...
    function _holdersBlock(
        ...
        (bool complete, address[] memory holders) = RolesProbes._tryEnumerateHolders(token, ownableGetterSig, role);
        if (!complete) {
        ...
        returns (address[] memory)
    {
        ...
        for (uint256 i = 0; i < candidates.length; i++) {
            bool member = acl
                ? RolesProbes._hasRole(token, role, candidates[i])
                : RolesProbes._isInOwnableSet(token, ownableIsSig, candidates[i]);
    ...
    function _union(address[] memory a, address[] memory b) private pure returns (address[] memory) {
        ...
        for (uint256 i = 0; i < a.length; i++) {
            if (a[i] != address(0) && !RolesProbes._contains(_shrink(out, n), a[i])) out[n++] = a[i];
        }
        for (uint256 i = 0; i < b.length; i++) {
            if (b[i] != address(0) && !RolesProbes._contains(_shrink(out, n), b[i])) out[n++] = b[i];
        }
        assembly {
            mstore(out, n)
        }
    ...
    /// tokenAdminRegistry`), e.g. a project registered in a different TokenAdminRegistry. Resolution: declared
        ...
        returns (string memory)
    {
        ...
        if (isV2) (, hooks) = RolesProbes._tryAddress(pool, "getAdvancedPoolHooks()");
        ...
        returns (string memory)
    {
        ...
        } else if (RolesProbes._looksLikeSafe(poolOwner)) {
            safe = poolOwner;
        ...
        } else if (RolesProbes._looksLikeTimelock(poolOwner)) {
            timelock = poolOwner;
