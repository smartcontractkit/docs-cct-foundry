...
library RolesProbes {
    ...
    function _tryUint(address target, string memory sig) internal view returns (bool ok, uint256 val) {
        (bool s, bytes memory ret) = target.staticcall(abi.encodeWithSignature(sig));
        if (s && ret.length >= 32) return (true, abi.decode(ret, (uint256)));
        return (false, 0);
    }

    function _tryBool(address target, string memory sig) internal view returns (bool ok, bool val) {
        (bool s, bytes memory ret) = target.staticcall(abi.encodeWithSignature(sig));
        if (s && ret.length >= 32) return (true, abi.decode(ret, (bool)));
        return (false, false);
    }

    function _tryBytes32(address target, string memory sig) internal view returns (bool ok, bytes32 val) {
        (bool s, bytes memory ret) = target.staticcall(abi.encodeWithSignature(sig));
        if (s && ret.length >= 32) return (true, abi.decode(ret, (bytes32)));
        return (false, bytes32(0));
    }

    function _tryAddressArray(address target, bytes memory callData)
        internal
        view
    ...
    function _detectTemplate(address token) internal view returns (TokenTemplate) {
        (bool hasAcl,) = _tryBytes32(token, "DEFAULT_ADMIN_ROLE()");
        if (hasAcl) {
            (bool hasRules,) = _tryAddress(token, "defaultAdmin()");
            return hasRules ? TokenTemplate.CrossChainToken : TokenTemplate.BurnMintERC20;
        }
        (bool hasOwner,) = _tryAddress(token, "owner()");
    ...
    function _templateName(TokenTemplate t) internal pure returns (string memory) {
    ...
    function _templateFromName(string memory name) internal pure returns (TokenTemplate) {
        bytes32 h;
        assembly {
    ...
    function _roleIdOrDefault(address token, string memory sig, bytes32 fallbackRole) internal view returns (bytes32) {
        (bool ok, bytes32 id) = _tryBytes32(token, sig);
    ...
    function _tokenRoleFromName(string memory name) internal pure returns (TokenRole) {
        bytes32 h;
        assembly {
            h := keccak256(add(name, 0x20), mload(name))
        }
    ...
    function _tokenRoleName(TokenRole role) internal pure returns (string memory) {
    ...
    function _requireRoleOnTemplate(TokenTemplate t, TokenRole role) internal pure {
    ...
    function _tokenRoleId(address token, TokenRole role) internal view returns (bytes32) {
        if (role == TokenRole.Minter) return _roleIdOrDefault(token, "MINTER_ROLE()", MINTER_ROLE);
        if (role == TokenRole.Burner) return _roleIdOrDefault(token, "BURNER_ROLE()", BURNER_ROLE);
        if (role == TokenRole.BurnMintAdmin) {
            return _roleIdOrDefault(token, "BURN_MINT_ADMIN_ROLE()", BURN_MINT_ADMIN_ROLE);
        }
    ...
    function _tryPendingOwner(address target) internal view returns (bool ok, address pending) {
        ...
        (bool hasOwner, address owner_) = _tryAddress(target, "owner()");
    ...
    function _isInOwnableSet(address token, string memory sig, address account) internal view returns (bool) {
    ...
    function _tryEnumerateHolders(address token, string memory ownableGetterSig, bytes32 role)
        internal
        view
        returns (bool complete, address[] memory holders)
    {
        if (bytes(ownableGetterSig).length != 0) {
            (bool okSet, address[] memory set) = _tryAddressArray(token, abi.encodeWithSignature(ownableGetterSig));
    ...
    function _readPoolAdmins(address pool)
        internal
        view
    ...
    function _looksLikeSafe(address candidate) internal view returns (bool) {
        if (candidate.code.length == 0) return false;
        (bool okT,) = _tryUint(candidate, "getThreshold()");
        if (!okT) return false;
        (bool okO,) = _tryAddressArray(candidate, abi.encodeWithSignature("getOwners()"));
        return okO;
    }

    /// @notice True when `candidate` answers the OZ TimelockController surface (getMinDelay).
    function _looksLikeTimelock(address candidate) internal view returns (bool) {
        if (candidate.code.length == 0) return false;
        (bool ok,) = _tryUint(candidate, "getMinDelay()");
        return ok;
    }
