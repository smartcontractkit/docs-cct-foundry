...
| `discover`       | List the CCIP API chain catalog (both planes) joined against local configs                                                                                                                                                                        | `FILTER=<term>`, `ENVIRONMENT=<testnet\|mainnet>` (both optional)                                                                                                                                              | `bash script/config/sync-discover.sh`                                                                                                                                                     |
...
| `sync`           | Refresh one chain's API-served fields (`ccip{}` or `ccipNative{}` + identity/metadata) from the API                                                                                                                                                     | `CHAIN=` (required)                                                                                                                                                     | `SyncCcipConfig.s.sol --sig "run(string)" <CHAIN>` → canonicalize                                                                                                                         |
...
| `probe-chain`    | READ-ONLY read of a chain's CCIP wiring over plain JSON-RPC, no fork (works where `doctor`'s fork backend cannot reach the chain; see [decision 0002](decisions/0002-eip-1898-fork-reads.md)). The forge script reports every rung and exits 0 even on `[FAIL] ... NO CODE`; the shell wrapper supplies the verdict. Make remaps it to pass/fail; CI calls the script directly for 0 `READABLE` / 1 `UNREADABLE` / 2 `UNRESOLVED` | `CHAIN=` (required) | `bash script/config/probe-chain.sh <CHAIN>` -> `ProbeChain.s.sol --tc ProbeChain --sig "run(string)" <CHAIN>` |
...
| `clean-scratch`  | Remove test-scratch fixtures (`zz-scratch-*`, `zz-tt-*`, `local-*`) from `config/chains`, `project`, `history`, and `batches` via explicit patterns - never `git clean -X`, which would also delete real gitignored project state | - | explicit `rm` patterns |
...
- **`config/chains/<selectorName>.json`** - pure API/chain facts. Everything the CCIP REST API serves
  ...
  `explorerUrl`/`nativeCurrencySymbol`, plus `ccipNative{}`/`nativeChainId` on non-EVM chains) is
  API-owned, written by the sync. The keys the API serves nothing
